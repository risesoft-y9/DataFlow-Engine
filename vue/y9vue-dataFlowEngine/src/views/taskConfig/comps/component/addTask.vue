...
const getDataAll = async () => {
    ...
    if (res) {
        ...
        if (
            ret.taskConfigModel.tableNumber != null &&
            ret.taskConfigModel.tableNumber != '' &&
            ret.taskConfigModel.tableNumber != undefined
        ) {
        ...
        if (
            ret.taskConfigModel.splitFactor != null &&
            ret.taskConfigModel.splitFactor != '' &&
            ret.taskConfigModel.splitFactor != undefined
        ) {
...
const rules = {
    ...
    targetId: [{ required: true, message: '请选择目的源', trigger: ['blur', 'change'] }]
};
let ruleFormRef = ref(addTaskFormRef);
</script>
<template>
    <el-form v-loading="addTaskLoading" ref="ruleFormRef" :model="addTaskForm.tableData" :rules="rules">
