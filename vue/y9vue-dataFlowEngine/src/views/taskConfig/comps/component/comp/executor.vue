...
const getData = async () => {
    let res = await getFindByTypeName({ typeName: 'executor' });
    if (res) {
        taskSetForm.executorList = res.data;
...
let rules = ref({
    className: [{ required: true, message: '请选择', trigger: ['blur', 'change'] }],
    value1: [],
    value2: []
});
...
<template>
    <div v-if="!showForm" v-loading="loading" style="width: 100%; height: 400px"></div>
    <el-form ref="ruleFormRef" :model="taskSetForm.tableData" :rules="rules" v-if="showForm">
        <div>
            <div v-for="items in taskSetForm.executorList">
