  ...
  >
    ...
    <template #dispatchType>
      <div class="y9select-task">
        ...
        <el-select
            clearable
            v-model="state.form.dispatchType"
            class="m-2"
            placeholder="请选择"
        >
          <el-option
    ...
    <template #queryFun>
      ...
      <div class="right-btn" v-if="props?.tableRow?.type!='dispatch'">
        <el-button class="global-btn-second" @click="lookLog"
        >{{ ('查询日志') }}
        </el-button>
        <el-button class="global-btn-second" @click="schedule"
        >{{ ('调度一次') }}
        </el-button>
...
const initParams = () => {
  if (props?.tableRow?.type == 'dispatch') { //调度日志的弹窗
