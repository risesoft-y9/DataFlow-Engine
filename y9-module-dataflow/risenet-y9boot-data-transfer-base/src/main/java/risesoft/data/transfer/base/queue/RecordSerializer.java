...
public class RecordSerializer implements Serializer<Record> {
	...
	@Override
	public byte[] serialize(Record item) throws IOException {
	...
	@Override
	public Record deserialize(byte[] data) throws IOException {
		Kryo kryo = kryoThreadLocal.get();
		return kryo.readObject(new Input(data), DefaultRecord.class);
	}
}