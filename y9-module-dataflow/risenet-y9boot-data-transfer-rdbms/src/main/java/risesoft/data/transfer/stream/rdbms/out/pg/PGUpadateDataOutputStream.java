...
/**
 * Update 操作的流 没有逻辑判断直接根据id修改数据 pg 使用
 * 
 * @typeName PGUpadateDataOutputStream
 * @date 2023年12月21日
 * @author lb
 */
public class PGUpadateDataOutputStream extends RdbmsDataOutputStream {
	...
	protected PreparedStatement fillPreparedStatement(PreparedStatement preparedStatement, Record record)
		...
		for (int i = 0; i < updateField.size(); i++) {
			...
			psHandle.fillPreparedStatementColumnType(preparedStatement, size + i + 1, column, dataBaseType,
					resultSetMetaData);
		}
