...
class StringCast {
	...
	static void init(final Configuration configuration) {
		...
		StringCast.extraFormats = configuration.getList("common.column.extraFormats", Collections.<String>emptyList(),
				String.class);
	...
	static String asString(final DateColumn column) {
		...
		default:
			throw TransferException.as(CommonErrorCode.CONVERT_NOT_SUPPORT,
					"时间类型出现不支持类型，目前仅支持DATE/TIME/DATETIME。该类型属于编程错误，请反馈 .");
		}
	}
}
...
class BytesCast {
	...
	static void init(final Configuration configuration) {
		BytesCast.encoding = configuration.getString("common.column.encoding", BytesCast.encoding);
		return;
	}
